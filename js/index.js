...
document.querySelector("body").onload = async function () {
	...
	console.log(server)
	if(server == "nova"){
		version.innerHTML = "| 1:4 map scans @ 13/7/2024 by Davide0033 | "

	} else version.innerHTML = "| 1:4 map scans @ 30/10/2022 by 3meraldK | "
}
...
function renderTowns() {
	...
	let mapURL = `.`,
		markers = `/images/`,
		worldMarker = `${server}_markers_earth.json`;
		
	
		function aa() {
			
			fetch(`${mapURL}${markers}${worldMarker}`)
				.then(res => res.json())
				.then(json => {
		...
		aa().catch(error => console.log(`Could not fetch config: ${error}`));
